...
export async function GET(req: NextRequest) {
  ...
  if (emotions.length > 0) {
    normalized.sort((a: Normalized, b: Normalized) => {
      const scoreA = emotions.reduce((s, e) => s + (a.emotionProfile.find((x: { emotion: string }) => x.emotion === e)?.score ?? 0), 0);
      const scoreB = emotions.reduce((s, e) => s + (b.emotionProfile.find((x: { emotion: string }) => x.emotion === e)?.score ?? 0), 0);
